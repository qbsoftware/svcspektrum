...
class Command(BaseCommand):
    ...
    def configure_site(self):
        ...
        site.organization = Organization.objects.order_by("id").first()
    ...
    def merge_course_registration_periods(self):
        self.registration_period_ids_map = get_ids_map()
        existing_registration_periods = {
            (period.registration_id, period.period_id): period.id
        ...
        for connection in import_connections:
            for foreign_registration_period in CourseRegistrationPeriod.objects.using(connection).all():
                ...
                if key in existing_registration_periods:
                    self.registration_period_ids_map[connection][foreign_registration_period.id] = (
                        existing_registration_periods[key]
                    )
                    continue
                ...
                self.registration_period_ids_map[connection][foreign_registration_period.id] = registration_period.id
        ...
        for model, name in [
            ...
            for connection in import_connections:
                for foreign_discount in model.objects.using(connection).all():
                    ...
                    if isinstance(discount, CourseDiscount):
                        discount.registration_period_id = self.registration_period_ids_map[connection][
                            discount.registration_period_id
                        ]
    ...
    def merge_timesheets(self):
        ...
        self.timesheet_ids_map = get_ids_map()
        timesheets_by_period_leader: dict[tuple[int, int], Timesheet] = {
            (ts.period_id, ts.leader_id): ts for ts in Timesheet.objects.using("default").all()
        }
        for connection in import_connections:
            for foreign_timesheet in Timesheet.objects.using(connection).all():
                period_id = timesheet_period_ids_map[connection][foreign_timesheet.period_id]
                leader_id = self.leader_ids_map[connection][foreign_timesheet.leader_id]
                key = (period_id, leader_id)
                timesheet = timesheets_by_period_leader.get(key)
                if timesheet is None:
                    timesheet = copy(foreign_timesheet)
                    timesheet.period_id = period_id
                    timesheet.leader_id = leader_id
                    timesheets_by_period_leader[key] = save(timesheet)
                elif timesheet.submitted != foreign_timesheet.submitted or timesheet.paid != foreign_timesheet.paid:
                    timesheet.submitted = timesheet.submitted and foreign_timesheet.submitted
                    timesheet.paid = timesheet.paid and foreign_timesheet.paid
                    timesheet.save(update_fields=["submitted", "paid"])
